...
# --------------------------------------------------------
# Modified training run to run only a single epoch and extract 
# the latent vectors from the dataset. Not the most elegant,
# but works.
# Launch with:
#    python --resume $CHECKPOINT_PATH$ --data_path $DATA_PATH$ 
#           --output_dir $OUTPUT_DIR$ --epoch $CHECKPOINT+1$
# File will be output to the --output_dir for my own sanity
# because my workspace is already so messy.
...
import models_mae_extract as models_mae
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        local_file = os.path.join(args.output_dir, 'latents-%s.pth' % epoch)
